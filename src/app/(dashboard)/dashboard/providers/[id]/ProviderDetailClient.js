...
export default function ProviderDetailPage() {
  ...
  const handleRetestOne = async (id) => {
    ...
    try {
        ...
        setConnections((prev) =>
          prev.map((c) =>
            c.id === id
              ? {
                  ...c,
                  testStatus: data.valid ? "active" : "unavailable",
                  lastError: data.valid ? null : data.error || null,
      ...
      console.log("Error retesting connection:", error);
    } finally {
      setRetestingId(null);
    }
  };

  // Object-based swap adapter for the rich panel (page handlers are index-based)
  const handleSwapConnections = (a, b) => {
    if (!a || !b) return;
    const ia = connections.findIndex((c) => c.id === a.id);
    const ib = connections.findIndex((c) => c.id === b.id);
    if (ia === -1 || ib === -1) return;
    handleSwapPriority(ia, ib);
  };

  const handleBatchSetActive = async (ids, active) => {
    ...
    setConnections((prev) => prev.map((c) => (ids.includes(c.id) ? { ...c, isActive: active } : c)));
  };

  const handleBatchRetestPanel = async (ids) => {
    ...
    try {
      for (const id of ids) {
        ...
        try {
          ...
          console.log("Error batch retesting", id, error);
        }
        setRetestingId(null);
      }
    } finally {
  ...
  const handleBatchDeletePanel = (ids) => {
    if (!ids || ids.length === 0) return;
    setConfirmState({
      title: `Delete ${ids.length} Connection${ids.length > 1 ? "s" : ""}`,
      ...
      onConfirm: async () => {
        ...
        setSelectedConnectionIds((prev) => prev.filter((id) => !ids.includes(id)));
        if (failed > 0) alert(`Deleted ${ids.length - failed} connection(s), ${failed} failed.`);
      },
    });
  };

  const handleToggleRateLimitPanel = async (id, enabled) => {
    ...
    try {
      const res = await fetch(`/api/providers/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ providerSpecificData: psd }),
      });
      if (res.ok) {
        setConnections((prev) =>
          prev.map((c) => (c.id === id ? { ...c, providerSpecificData: psd } : c))
        );
      }
    } catch (error) {
      console.log("Error toggling rate limit protection:", error);
    }
  };

  const handleToggleQuotaPanel = async (id, visible) => {
    const conn = connections.find((c) => c.id === id);
    const psd = { ...(conn?.providerSpecificData || {}) };
    ...
    try {
      ...
      console.log("Error toggling quota visibility:", error);
    }
  };
  ...
  // Rich connections panel — Phase 2: pagination, search/filter, bulk
  // operations, error classification, cooldown timers, rate-limit protection,
  // proxy pool assignment, and per-row inline actions.
  const connectionsList = (
    <ConnectionsListPanel
      ...
      oneByOneStatuses={oneByOneResults}
      controlledSelectedIds={new Set(selectedConnectionIds)}
      onSelectionChange={(s) => setSelectedConnectionIds(Array.from(s))}
      onToggleActive={(id, isActive) => handleUpdateConnectionStatus(id, isActive)}
      onToggleRateLimit={(id, enabled) => handleToggleRateLimitPanel(id, enabled)}
      onToggleQuotaVisibility={(id, visible) => handleToggleQuotaPanel(id, visible)}
      onSwapPriority={handleSwapConnections}
      onRetest={(id) => handleRetestOne(id)}
      onEdit={(conn) => {
      ...
      onUpdateProxy={async (connId, proxyPoolId) => {
        try {
      ...
      onBatchSetActive={handleBatchSetActive}
      onBatchRetest={handleBatchRetestPanel}
      onBatchDelete={handleBatchDeletePanel}
    />
  );
